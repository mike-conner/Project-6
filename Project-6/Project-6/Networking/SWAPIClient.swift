...
class SWAPIClient {
    ...
    func getEntityList(entityType: EntityType, completionHandler completion: @escaping (People?, Vehicles?, StarShips?, Error?) -> Void) {
        ...
        guard let url = URL(string: entityType.rawValue, relativeTo: base) else {
            completion(nil, nil, nil, SWAPIError.invalidUrl)
            return
        }
            ...
            DispatchQueue.main.async {
                if let data = data {
                    guard let httpResponse = response as? HTTPURLResponse else {
                        completion(nil, nil, nil, SWAPIError.requestFailed)
                        return
                    }
                    if httpResponse.statusCode == 200 {
                        do {
                            ...
                            case .people:
                                ...
                                completion(entities, nil, nil, nil)
                            case .vehicles:
                        ...
                        completion(nil, nil, nil, SWAPIError.invalidData)
                    }
